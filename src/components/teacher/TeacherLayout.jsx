...
const TeacherLayout = ({
    ...
    const SidebarContent = () => (
        <div className="flex flex-col h-full">
            <div className="p-6 flex items-center gap-3 border-b border-gray-100/50 bg-white/30 backdrop-blur-sm">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
                {menuItems.map((item, idx) => {
                    if (item.section) {
                        return (
                            <p key={idx} className="px-4 text-xs font-extrabold text-gray-400 uppercase tracking-wider mb-2 mt-6">
                                {item.section}
                            </p>
                        );
                    }
                    ...
                    return (
                        ...
                        >
                            ...
                            {item.badge > 0 && (
                                <span className={`ml-auto ${item.badgeColor} px-2.5 py-0.5 rounded-full text-xs font-extrabold shadow-sm`}>
                                    {item.badge}
                                </span>
                            )}
                        </button>
            ...
            <div className="p-4 border-t border-gray-100/50 bg-white/30 backdrop-blur-sm">
                <div className="flex items-center gap-3 px-4 py-3 mb-2 rounded-xl bg-white/50 border border-white shadow-sm">
                ...
                <button
                    onClick={onLogout}
                    className="w-full flex items-center gap-3 px-4 py-3 text-red-500 hover:bg-red-50 hover:text-red-600 rounded-xl font-bold transition-colors"
                >
    ...
    return (
        <div className="flex min-h-screen bg-[#F3F4F6] font-sans text-gray-900 overflow-hidden relative selection:bg-indigo-100 selection:text-indigo-700">
            ...
            {/* Desktop Sidebar */}
            <aside className="hidden md:block w-72 bg-white/60 backdrop-blur-xl border-r border-white/50 h-screen sticky top-0 z-30 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
                <SidebarContent />
            </aside>
